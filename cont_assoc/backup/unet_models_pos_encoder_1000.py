...
import cont_assoc.utils.save_features as sf                # modified
...
from cont_assoc.utils.assoc_module_with_cu import AssociationModule
...
class UNet(LightningModule):
    def __init__(self, cfg):
        ...
        self.sparse_encoder_v2 = SparseEncoder_v2(cfg)
        ...
        self.AssocModule = AssociationModule(weights, thresholds, self.sparse_encoder_v2,
    ...
    def forward(self, x):
        ...
        # coors = [torch.from_numpy(i).type(torch.LongTensor).cuda()
        #         for i in x['pt_cart_xyz']]
        # feats = [torch.from_numpy(i).type(torch.LongTensor).cuda()
        #         for i in x['feats']]

        # pos_features = self.sparse_tensor(coors, feats)[0]
        # x['feats'] = [pos_features.cpu().numpy().astype(np.float32)]


        coordinates, voxel_features = self.voxel_feature_extractor(x)

        voxel_features_pos = self.sparse_tensor([coordinates[:,1:]], [voxel_features])[0]
        batch_size = len(x['grid'])
        ins_features = self.sparse_encoder_v2(coordinates, voxel_features_pos, batch_size)                    
    ...
    def group_instances(self, norm_features, pos_labels, sem_labels, pos_scans):
        ...
        for i in range(len(ids)): #iterate over all instances
            if ins_num > 1000:
                break    
            if sorted_n_ids[i] < 100: #filter too small instances 30
            ...
            elif sorted_n_ids[i] >= 100 and sorted_n_ids[i] <= 200:
                ...
                _feats.append(feat)
                _sem_labels.append(s_labels)
            elif sorted_n_ids[i] > 200 and sorted_n_ids[i] <= 800:
                pick_num = 100
                ins_num += pick_num
                pt_idx = torch.where(pos_labels==ids[i])[0]
                pt_idx = pt_idx[torch.randperm(sorted_n_ids[i])][:pick_num]
...
# Modules
class SparseEncoder_v2(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        self.voxel_feature_extractor = blocks.VoxelFeatureExtractor(cfg)          #mm
        self.encoder = CylinderEncoder(cfg)
