...
def fix_batches_pos(ins_ids, features, coordinates, grid_coordinates):
    new_feats = []
    new_coors = []
    ...
    for i in range(len(ins_ids)):
        ...
        if len(ins_ids[i]) == 0:
            new_feats.append([])
            new_coors.append([])
            new_grid_coordinates.append([])
        else:
            new_feats.append(features[c])
            new_coors.append(coordinates[c])
            new_grid_coordinates.append(coordinates_T[c])
            c += 1
    return new_feats, new_coors, new_grid_coordinates
